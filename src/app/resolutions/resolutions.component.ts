...
export class ResolutionsComponent implements OnInit {
          ...
          if (!data.error) {
            if (data.results_count) {
              ...
              itemData.formData = {
                ...
                remarks: res.remarks,
                session_date: this.textService.formatDefaultDate(
                  res.session_date
                ),
  ...
  showDialog(data): void {
    const dialogRef = this.dialog.open(ResolutionModalComponent, {
      data: { ...data },
