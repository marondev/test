...
const checkIDExist = function (req, res, next) {
  Memorandum.findByPk(req.params.id).then((e) => {
    if (e && e.length !== 0) {
...
router.get("/", function (req, res) {
  ...
  const order_field = req.query.order_field || "executed_date";
...
router.post("/", function (req, res) {
  ...
  Memorandum.create(
    {
      approved_date: parseInt(
        moment(req.body.approved_date).format("YYYYMMDD")
      ),
      ...
      other_party: req.body.other_party,
...
router.get("/:id", [checkIDInput, checkIDExist], function (req, res) {
  Memorandum.findByPk(req.params.id, {
    include: [{ all: true }],
  }).then((result) => {
    params.results = result;
    params.results_count = 1;
...
router.put("/:id", [checkIDInput, checkIDExist], function (req, res) {
  req.body.approved_date = parseInt(
    moment(req.body.approved_date).format("YYYYMMDD")
  );

  Memorandum.update(req.body, {
